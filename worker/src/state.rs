...
///
/// Keys:
///   device:{device_id}:server       -> worker domain holding the connection
///   device:{device_id}:pending      -> JSON list of unacked commands
///   device:{device_id}:last_ack     -> last command ID the phone confirmed
///   device:{device_id}:cmd_counter  -> next command ID to assign
pub struct State {
    redis: redis::Client,
    worker_id: String,
}
...
impl State {
    ...
    /// Register this connection: set the worker holding this device's session.
    pub async fn register_connection(&self, device_id: &str) -> Result<(), redis::RedisError> {
        let mut conn = self.conn().await?;
        let key = format!("device:{device_id}:server");
        conn.set::<_, _, ()>(&key, &self.worker_id).await?;
        info!(device_id, worker = %self.worker_id, "registered connection");
        Ok(())
    }

    /// Clean up when a phone disconnects.
    pub async fn unregister_connection(&self, device_id: &str) -> Result<(), redis::RedisError> {
        let mut conn = self.conn().await?;
        let key = format!("device:{device_id}:server");
        ...
        if current.as_deref() == Some(&self.worker_id) {
            ...
            info!(device_id, "unregistered connection");
        }
        Ok(())
    }

    /// Get the last ack value stored in Redis.
    pub async fn get_last_ack(&self, device_id: &str) -> Result<i64, redis::RedisError> {
        let mut conn = self.conn().await?;
        let key = format!("device:{device_id}:last_ack");
        let val: Option<i64> = conn.get(&key).await?;
        Ok(val.unwrap_or(0))
    }

    /// Update last_ack and remove acked commands from pending.
    pub async fn process_ack(&self, device_id: &str, ack_id: i64) -> Result<(), redis::RedisError> {
        let mut conn = self.conn().await?;

        // Update last_ack
        let ack_key = format!("device:{device_id}:last_ack");
        conn.set::<_, _, ()>(&ack_key, ack_id).await?;

        // Remove acked commands from pending list
        let pending_key = format!("device:{device_id}:pending");
        ...
        info!(device_id, ack_id, "processed ack");
        Ok(())
    }
    ...
    pub async fn get_pending_commands(
        ...
        info!(device_id, since_ack, count = replay.len(), "pending commands to replay");
        Ok(replay)
    }
    ...
    pub async fn enqueue_command(
        ...
        let counter_key = format!("device:{device_id}:cmd_counter");
        ...
        info!(device_id, id, "enqueued command");
        Ok(command)
    }
    ...
    pub async fn store_response(
        ...
        let mut conn = self.conn().await?;
        let key = format!("device:{device_id}:response:{cmd_id}");
