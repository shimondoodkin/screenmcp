"""PhoneMCP Python client — async SDK for controlling Android phones."""
...
logger = logging.getLogger("phonemcp")
...
class PhoneMCPError(Exception):
    """Base exception for PhoneMCP errors."""


class AuthError(PhoneMCPError):
    """Raised when authentication fails."""


class ConnectionError(PhoneMCPError):  # noqa: A001  (shadows builtin intentionally)
    """Raised when the WebSocket connection is lost or unavailable."""


class CommandError(PhoneMCPError):
    """Raised when a phone command returns an error status."""


class PhoneMCPClient:
    """Async client for the PhoneMCP phone-control platform.
    ...
    ----------
    api_key:
        A ``pk_``-prefixed API key issued from the PhoneMCP dashboard.
    api_url:
        Base URL of the PhoneMCP web API.
    device_id:
    ...
    async def __aenter__(self) -> "PhoneMCPClient":
        await self.connect()
        return self
    ...
    async def _discover(self) -> str:
        ...
        if not ws_url:
            raise PhoneMCPError("discovery returned no wsUrl")
        return ws_url
    ...
    async def _connect_ws(self, worker_url: str) -> None:
        ...
        if msg.get("type") == "auth_ok":
            ...
            raise PhoneMCPError(f"unexpected auth response: {msg}")
