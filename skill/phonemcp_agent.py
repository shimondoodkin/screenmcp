#!/usr/bin/env python3
"""PhoneMCP AI Agent -- control an Android phone with natural language.

Usage:
    python phonemcp_agent.py "Open Settings and turn on Wi-Fi"

Environment variables:
    PHONEMCP_API_KEY   -- PhoneMCP API key (required, starts with pk_)
    ANTHROPIC_API_KEY  -- Anthropic API key (required)
    PHONEMCP_API_URL   -- PhoneMCP server URL (default: https://server10.doodkin.com)
    PHONEMCP_DEVICE_ID -- Target device UUID (optional)
    PHONEMCP_MAX_STEPS -- Maximum agent loop iterations (default: 15)
"""
...
PHONEMCP_API_KEY = os.environ.get("PHONEMCP_API_KEY", "")
PHONEMCP_API_URL = os.environ.get("PHONEMCP_API_URL", "https://server10.doodkin.com")
PHONEMCP_DEVICE_ID = os.environ.get("PHONEMCP_DEVICE_ID", "")
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")
MAX_STEPS = int(os.environ.get("PHONEMCP_MAX_STEPS", "15"))
...
async def execute_action(
    phone: PhoneMCPClient, action: str, params: dict[str, Any]
) -> str:
...
async def run_agent(instruction: str) -> None:
    """Main agent loop."""
    if not PHONEMCP_API_KEY:
        print("Error: PHONEMCP_API_KEY environment variable is required.")
        sys.exit(1)
    ...
    print(f"Connecting to PhoneMCP ({PHONEMCP_API_URL})...")

    async with PhoneMCPClient(
        api_key=PHONEMCP_API_KEY,
        api_url=PHONEMCP_API_URL,
        device_id=PHONEMCP_DEVICE_ID or None,
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python phonemcp_agent.py \"<instruction>\"")
        print("Example: python phonemcp_agent.py \"Open Settings and turn on Wi-Fi\"")
        sys.exit(1)
